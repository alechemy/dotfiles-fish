...
if [ -f "$NAVIDROME_ENV" ]; then
  ...
  echo "Navidrome config not found at $NAVIDROME_ENV"
  exit 1
fi
USERNAME="${NAVIDROME_USERNAME:-alec}"
PASSWORD=$(security find-generic-password -s 'Navidrome' -a "$USERNAME" -w 2>/dev/null)
if [ -z "$PASSWORD" ]; then
  echo "Navidrome password not found in Keychain."
  echo "Add it with: security add-generic-password -s 'Navidrome' -a 'alec' -w"
  exit 1
fi
MIN_TRACKS=5
MAX_TRACKS=30
