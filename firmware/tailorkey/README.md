...
- Stock Glove80's right thumb cluster has a Control key bound to `LCTRL` for the same reason (Tweak 1 displaces that key in the working file, but the convention still applies when reading `upstream/`).
...
Seven tweaks layered on stock TailorKey. The unmodified reference sits at `upstream/Glove80 TailorKey v5.2³ Bilateral - macOS.json` so this inventory can be regenerated by diff.

#### Tweak 1: mouse buttons on the left cluster's inner thumb keys

Layer 0. The inner key of each **left** thumb-cluster row is a mouse button — back (top) row = left click, front (bottom) row = right click — so the left thumb clicks for the trackball mounted above the right cluster. The right cluster's inner keys deliberately carry no click bindings: clicking is left-thumb-only, an RSI guard so the right hand stays on the trackball instead of click-reaching.

| Pos | Cluster | Row   | Was         | Now         | Role        |
|-----|---------|-------|-------------|-------------|-------------|
| 54  | Left    | Back  | `&lower`    | `&mkp LCLK` | Left click  |
| 71  | Left    | Front | `&kp LALT`  | `&mkp RCLK` | Right click |
| 55  | Right   | Back  | `&kp LCTRL` | `&lower`    | Lower-layer toggle, rehomed from pos 54 |

Knock-on effects:

- The left thumb Alt (71) and the right thumb Control (55) are gone; those modifiers flow through the home-row mods (left: `A`=Ctrl, `S`=Option, `D`=CMD, `F`=Shift; right: `J`=Shift, `K`=CMD, `L`=Option, `;`=Ctrl).
- No middle-click binding exists on Layer 0; the Mouse layers (14–17) still carry `MCLK`.

Everything else on both clusters is stock: 52/53 (`LSHFT`/`LGUI`), 56 (`RGUI`, used by the Shyper combo in Tweak 6), 57 (`RSHFT`), 69/70 (Backspace/Delete), 72–74 (Option/Enter/Space).
...
Layer 18 (Lower) top row, all positions remapped to `F1`–`F10` to recover access to the F-row that the base layer's media keys displace. (`&lower`, the layer's only entry point, lives at pos 55 — see Tweak 1.)
...
`keyPositions` shifted from upstream `[52, 71]` to `[54, 71]`: the left cluster's two mouse-click keys (Tweak 1) pressed together fire the Alt-Tab switcher.

#### Tweak 5: removed `right_meh_ACS_v1_TKZ` combo

Stock TailorKey defines a sticky-Meh (Alt+Ctrl+Shift) combo on `keyPositions: [56, 73]`, which in stock TailorKey means RGUI + Enter. The combo was unused in the Karabiner / AeroSpace stack here, and Tweak 6 reclaims the same positions for Shyper. Removed first so positions [56, 73] are available without overlap.

#### Tweak 6: added Shyper combo

Second combo beyond stock TailorKey, in the `combos` array:

```
right_shyper_RGUI_v1_TKZ
  binding:      &sk RGUI                (sticky right_command)
  keyPositions: [56, 73]                (RGUI thumb + Enter thumb, the old Meh positions)
  layers:       [0, 2]
```
...
#### Tweak 7: second Hyper trigger on Space + Enter (displaces right caps-word)

Adds a second sticky-Hyper combo on the right thumb front row, replacing the stock `right_caps_word_v1_TKZ` combo:

```
right_hyper_ACSG_space_enter_v1_TKZ
  binding:      &sk LA(LC(LS(LGUI)))    (sticky Alt+Ctrl+Shift+Cmd, same as Tweak 3)
  keyPositions: [73, 74]                (Enter thumb front + Space thumb front)
  layers:       [0, 2]
```

Rationale: the Go60 has only a single-row thumb cluster, so its hyper combo lives on Enter + Space (the two rightmost right-thumb keys; see Go60 Tweak C). Adding the same chord here means the gesture transfers across boards without retraining. The original Glove80 Hyper combo on `[57, 74]` (Shift + Space — Tweak 3) is retained, since that's the muscle memory established before the Go60 entered the picture.

Cost: the stock `right_caps_word_v1_TKZ` combo on `[73, 74]` was removed to free the positions. Caps-word remains reachable via `left_caps_word_v1_TKZ` on `[69, 70]` (Backspace + Delete thumb), which is untouched.

Net combo count: 12 (upstream's 11 + Tweak 3's Hyper + Tweak 6's Shyper + Tweak 7's second Hyper − Tweak 5's removed Meh − Tweak 7's removed right caps-word).
...
`sticky_hyp_rght_v1_TKZ` `keyPositions` moved from upstream `[43, 51]` (M + Up, on the bottom alpha row and nav row) to `[58, 59]` (Enter + Space, the two rightmost keys of the right thumb cluster). Much friendlier chord, and a muscle-memory mate for the Glove80's Space + Enter hyper trigger (see Glove80 Tweak 7). The Go60 has only a single-row thumb cluster, so the two rightmost thumb keys are the natural chord; the Glove80 reaches the same gesture from its front-row thumb keys.
