...
Five tweaks layered on stock TailorKey. The unmodified reference sits at `upstream/Glove80 TailorKey v5.2³ Bilateral - macOS.json` so this inventory can be regenerated by diff.
...
Stock TailorKey defines a sticky-Meh (Alt+Ctrl+Shift) combo on `keyPositions: [56, 73]`, which in stock TailorKey means RGUI + Enter. Position 56 is now `&mkp RCLK` from Tweak 1, so the combo would have fired on a right-click + Enter chord. Unlikely but plausible (e.g., clicking a context-menu item and immediately pressing Enter). Removed entirely since Hyper (Tweak 3) already covers the power-user-modifier slot. Net combo count: 11 (down from upstream's 11; we added Hyper in Tweak 3 and removed Meh here).
