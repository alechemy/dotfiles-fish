...
- **Watched folder:** `~/Dropbox (Maestral)/onyx/NoteMax/Notebooks`, recursing into any category subfolders. The leaf is the device model, so it changes when the Boox is replaced — `dt-watchdog.sh`'s stale-export check hardcodes the same path and must be updated with it. Files arrive via Maestral sync; the watcher acts on both `Created` and `Renamed` fswatch events because a sync client can finalize a downloaded file by renaming it into place — `--event Created` alone would miss those.
