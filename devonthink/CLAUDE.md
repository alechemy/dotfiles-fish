...
Boox device → Dropbox (via Boox export)
  → boox-import-watcher.sh (launchd + fswatch on the Maestral-synced Notebooks folder; deletes untitled `Notebook-<n>` quick notes) → boox-stage.sh (byte-hash dedup, atomic copy into ~/.local/state/devonthink/boox/staging/) → boox-process.py (launchd, AC/memory-pressure/driver-gated): renders pages, diffs by pixel signature, OCRs only new/changed pages through the local oMLX vision model — handwritten content NEVER reaches DT chat or DT's cloud OCR. Then, per notebook type:
...
SingleFile ingestion is OUT of smart rules — it's Python scripts + an fswatch launchd agent. See devonthink/README.md → "SingleFile Ingestion Pipeline".
  ...
  → Post-Enrich & Archive (action items → Things 3, daily notes extraction, briefing-event match via brief_events.py — LinkedEvent + sub-bullet under the matched 📅 event bullet — or its own timed bullet at its chronological position in the daily-note timeline, archive to 99_ARCHIVE) → move only on success
```
...
contact tracking or the birthdays digest (TRMNL snapshot) is silently dead — see
`docs/entities.md`.
