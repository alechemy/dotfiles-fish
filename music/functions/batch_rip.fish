function batch_rip -d "Batch music downloader with cooldown and logging"
    ...
    set input_file (test -n "$argv[2]"; and echo $argv[2]; or echo "$DOTFILES/music/downloads.txt") # Default input file
    set log_file "$DOTFILES/music/logs/rip_"(date +%Y%m%d_%H%M%S)".log"
    set error_log "$DOTFILES/music/logs/rip_errors_"(date +%Y%m%d_%H%M%S)".log"
